package com.coronalabs.extendingui;
...
public class ToolBar extends android.widget.RelativeLayout {
	...
	private final android.widget.ToggleButton fBlackButton;
	
	/** Button for changing the Corona background color to white. */
	private final android.widget.ToggleButton fWhiteButton;
	
	/** Button for changing the Corona background color to red. */
	private final android.widget.ToggleButton fRedButton;
	
	/** Button for changing the Corona background color to green. */
	private final android.widget.ToggleButton fGreenButton;
	
	/** Button for changing the Corona background color to blue. */
	private final android.widget.ToggleButton fBlueButton;
	...
	public ToolBar(android.content.Context context) {
		...
		fBlackButton.setTag(android.graphics.Color.BLACK);
		fBlackButton.setText(R.string.color_black);
		...
		fWhiteButton.setTag(android.graphics.Color.WHITE);
		fWhiteButton.setText(R.string.color_white);
		...
		fRedButton.setTag(android.graphics.Color.RED);
		fRedButton.setText(R.string.color_red);
		...
		fGreenButton.setTag(android.graphics.Color.GREEN);
		fGreenButton.setText(R.string.color_green);
		...
		fBlueButton.setTag(android.graphics.Color.BLUE);
		fBlueButton.setText(R.string.color_blue);
	...
	private class ColorButtonEventHandler implements android.view.View.OnClickListener {
		...
		public void onClick(android.view.View view) {
			// Check the toggle button that matches the given color and un-check the other toggle.
			...
			final int backgroundColor = (Integer) view.getTag();
			...
			if (activity != null) {
				activity.getRuntimeTaskDispatcher().send(new com.ansca.corona.CoronaRuntimeTask() {
					...
					public void executeUsing(com.ansca.corona.CoronaRuntime runtime) {
						...
						luaState.pushNumber(android.graphics.Color.red(backgroundColor) / 255.0);
						luaState.pushNumber(android.graphics.Color.green(backgroundColor) / 255.0);
						luaState.pushNumber(android.graphics.Color.blue(backgroundColor) / 255.0);
						luaState.call(4, 0);
						luaState.pop(1);
					}
				});
